  ...
  function markingFontBarBlock() {
    ...
    if (selection.isCollapsed()) {
     ...
     return externalES 
      ...
      if (startKey) {
        const allBlocks = externalES.getCurrentContent()
        let startBlock = allBlocks.getBlockForKey(startKey)
        ...
        // newSelection = newSelection.merge(selection)

        externalES = EditorState.forceSelection(externalES, selection)

        return externalES //= EditorState.push(externalES, newContent, 'change-block-type');
      }

...
const makingStyleObj = function (theme) {
  ...
  return {
    avatarCss: ({ size = "40px", personName, ...props }) => {
    ...
    chipCss: ({ size = theme.textSizeArr, personName, label, ...props }) => {
      const size_ = Array.isArray(size) ? size : [size]
      ...
      return {
        ...
        "& .MuiChip-avatar": {
          ...
          ...breakpointsAttribute(["width", ...avatarSize_], ["height", ...avatarSize_]), //avatar size

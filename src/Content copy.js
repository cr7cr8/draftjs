    ...
    const html = ReactHtmlParser(preHtml, {
      ...
      transform: function transformFn(node, index) {
        ...
        if (node.name === "linkoff") {
          ...
          return  <span key={index}><LinkTag host={node.attribs.linkhost} address={node.attribs.linkaddress} arr={arr} /></span>
    ...
    >
      ...
      <div style={{
        ...
        {arr.map((element, index) => {
          ...
          if (elementType === "emoji" || elementType === "longmentionoff_head" || elementType === "longmentionoff_body" || elementType === "linkoff") {
            return <span key={index}>{toHtml(reactElementToJSXString(element), imgArr)}</span>
          }
          else {
            return <span key={index}>{element}</span>
          }
          ...
          //   ? element //toHtml(reactElementToJSXString(element), imgArr)
          //   : element
